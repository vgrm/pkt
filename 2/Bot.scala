...
object ControlFunction
{
    ...
    def forMaster(bot: Bot) {
        ...
        var direction = BFS(bot,bot.view)
        var temp = XY(0,0)
        if(temp == direction){
        ...
        if(dontPlantLandMineUntil < bot.time && bot.energy > 1000){
            nearestEnemyMaster match {
                ...
                case Some(relPos) =>    // a master is nearby
                    ...
                    bot.spawn(unitDelta, "mood" -> "LandMine", "target" -> remainder)
                    bot.set("dontPlantLandMineUntil" -> (bot.time + relPos.stepCount + 1))
            }
        }
        ...
        if(dontSpawnGathererUntil < bot.time && bot.energy > 100){
            ...
            bot.set("dontSpawnGathererUntil" -> (bot.time + 10))
        }
    }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            ...
            case "Lurking" => reactAsLurker(bot)
    ...
    def BFS(bot: Bot, view: View) : XY = {
		...
		while (!queue.isEmpty) {
			...
			if (next.length > 15) {
                //  if no awailable path found do not move
				return XY(0, 0)
			}
			...
			for (i <- -1 to 1; j <- -1 to 1) {
					...
					if (cell == '_' && !visited.contains(xy)) {
						queue.enqueue(xy)
						visited += xy
						path += (xy -> next)
					}
                    if(cell == 'b' && xy.length < 1.5){
                        //  implement:
                        //  run from bad mobs??
                    }
			}
    ...
    def reactAsGatherer(bot: MiniBot){
        ...
        if(bot.energy > gather){
            bot.set("mood" -> "Lurking", "target" -> "")
            reactAsLurker(bot)
        }
        else if(bot.energy > gather/10 && !master.isEmpty){
            bot.set("mood" -> "Lurking", "target" -> "")
            reactAsLurker(bot)
        }
        else if(!nearestEnemyMaster.isEmpty && bot.energy < gather/10){
            bot.set("mood" -> "Aggressive", "target" -> "")
            ...
            val bestDirection45 = directionValue.zipWithIndex.maxBy(_._1)._2
            val direction = XY.fromDirection45(bestDirection45)
            //val direction = BFS(bot,bot.view)
            bot.move(direction)
            bot.set("lastDirection" -> bestDirection45)
        }
    }

    def reactAsLurker(bot: MiniBot){
        val (directionValue, nearestEnemyMaster, master) = analyzeViewAsLurker(bot, bot.view)

        val gather = bot.inputAsIntOrElse("gather", 0)
        if(bot.energy < gather && master.isEmpty){
        ...
        if(bot.view(masterDirectionLocal) == 'W' || bot.view(masterDirectionLocal) == 'p' || bot.view(masterDirectionLocal) == 'b'){
        ...
        val direction = XY.fromDirection45(bestDirection45)
        bot.move(direction)
        //bot.say(bot.view(masterDirectionLocal).toString())
    }
    ...
    def analyzeViewAsLurker(bot: MiniBot, view: View) = {
        val directionValue = Array.ofDim[Double](8)
        var nearestEnemyMaster: Option[XY] = None
        var master: Option[XY] = None
